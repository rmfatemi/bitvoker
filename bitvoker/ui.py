    ...
    if request.method == "POST":
        try:
            ...
            for channel in ["telegram", "slack", "discord", "gotify"]:
                ...
                if channel == "telegram":
                    new_config_data[channel]["bot_token"] = request.form.get(
                        f"channel_{channel}_config_bot_token",
                        new_config_data.get(channel, {}).get("bot_token", ""),
                    )
                    new_config_data[channel]["chat_id"] = request.form.get(
                        f"channel_{channel}_config_chat_id",
                        new_config_data.get(channel, {}).get("chat_id", ""),
                    )
                elif channel == "slack":
                    new_config_data[channel]["webhook_url"] = request.form.get(
                        f"channel_{channel}_config_webhook_url",
                        new_config_data.get(channel, {}).get("webhook_url", ""),
                    )
                    new_config_data[channel]["token"] = request.form.get(
                        f"channel_{channel}_config_token",
                    ...
                    new_config_data[channel]["server_url"] = request.form.get(
                        f"channel_{channel}_config_server_url",
                        new_config_data.get(channel, {}).get("server_url", ""),
                    )
                    new_config_data[channel]["app_token"] = request.form.get(
                        f"channel_{channel}_config_app_token",
                        new_config_data.get(channel, {}).get("app_token", ""),
                    )
