...
class Notifier:
        ...
        try:
            ...
            if not response.json().get("ok"):
                logger.error(
                    ...
                    config.get("name", "Telegram"),
                    response.json().get("description"),
                )
                return
            logger.info(
                "Successfully sent Telegram message for %s",
                config.get("name", "Telegram"),
            )
            ...
            logger.error(
                "Non-request error sending Telegram message for %s: %s",
                config.get("name", "Telegram"),
                ex,
            )
    ...
    def _send_discord(self, config, message, title):
        ...
        try:
            ...
            response.raise_for_status()
            logger.info(
                "Successfully sent Discord message for %s",
    ...
    def _send_gotify(self, config, message, title):
        ...
        try:
            ...
            logger.error(
                "Failed to send Gotify message for %s: %s",
                config.get("name", "Gotify"),
                e,
            )
    ...
    def send_message(self, message_body, title="Bitvoker Notification"):
        ...
        for channel_conf_wrapper in self.channels_config:
            if not channel_conf_wrapper.get("enabled", False):
                logger.debug(
                    "Skipping disabled channel: %s",
                    channel_conf_wrapper.get("name", "Unnamed Channel"),
                )
                continue
            ...
            if not specific_config:
                logger.warning(
                    "Channel %s is enabled but has no specific config.",
                    channel_conf_wrapper.get("name"),
                )
                continue
            ...
            try:
                if channel_type == "telegram":
                    if "bot_token" in specific_config and "chat_id" in specific_config:
                        ...
                        logger.error(
                            "Telegram channel %s missing bot_token or chat_id.",
                    ...
                    if "webhook_url" in specific_config:
                        ...
                        logger.error(
                            "Slack channel %s missing webhook_url.",
                            specific_config["name"],
                        )
                    ...
                    if "webhook_url" in specific_config:
                        ...
                        logger.error(
                            "Discord channel %s missing webhook_url.",
                            specific_config["name"],
                        )
                    ...
                    if "server_url" in specific_config and "app_token" in specific_config:
                        ...
                        logger.error(
                            "Gotify channel %s missing server_url or app_token.",
                            specific_config["name"],
                        )
                else:
                    logger.warning(
                        "Unsupported notification channel type: %s for %s",
                        channel_type,
                        specific_config["name"],
                    )
            except Exception as e:
                logger.error(
                    "Unhandled error processing channel %s (%s): %s",
                    specific_config["name"],
                    channel_type,
                    e,
                )
